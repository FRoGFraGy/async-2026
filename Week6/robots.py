...
async def reset_factory(client: httpx.AsyncClient):
    ...
    resp = await client.post(
            f"{BASE_URL}/student/{STUDENT_ID}/reset"
    )
...
async def grab_part(client: httpx.AsyncClient, robot_id: str, part: str):
    ...
    resp = await client.post(
            f"{BASE_URL}/student/{STUDENT_ID}/robot/{robot_id}/grab",
            json={"part": part}
    )
...
async def main():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=10.0) as client:
        ...
        t1 = asyncio.create_task(run_robot_task(client, "robot_1"))
        t2 = asyncio.create_task(run_robot_task(client, "robot_2"))
        t3 = asyncio.create_task(run_robot_task(client, "robot_3"))
        t4 = asyncio.create_task(run_robot_task(client, "robot_4"))
