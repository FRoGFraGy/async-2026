...
async def main():
    # case A: Using gather() to run tasks concurrently and collect results in order
    ...
    # case B: Using wait() to run tasks concurrently but only return the first completed task
    ...
    # clean up the loser task still running in the background
    for t in pending:
        t.cancel()
